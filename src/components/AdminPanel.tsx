...
export default function AdminPanel({
  ...
  const [adminSubTab, setAdminSubTab] = useState<'roster' | 'departments' | 'backups' | 'branding'>('roster');
    ...
    <div className="w-full bg-[#0b0c10]/40 border border-white/5 rounded-2xl p-6 glass-heavy font-sans" id="admin-panel-container">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-sm font-bold uppercase tracking-wider text-white">
              {adminSubTab === 'roster' 
                ? (isAr ? 'لوحة إدارة رموز المرور وهوية الطاقم' : 'Crew Credentials & Signatures Matrix')
                : adminSubTab === 'backups'
          ...
          {adminSubTab === 'roster' && (
            <button
              onClick={() => {
                ...
                if (!isAddingNewUser) setSelectedUserId(null);
              }}
              className="bg-indigo-600 hover:bg-indigo-500 text-white font-bold text-xs px-3.5 py-2 rounded-xl flex items-center gap-1.5 transition-all cursor-pointer shadow-md"
            >
              {isAddingNewUser ? <X className="h-3.5 w-3.5 shrink-0" /> : <UserPlus className="h-3.5 w-3.5 shrink-0" />}
              <span>{isAddingNewUser ? (isAr ? 'إلغاء' : 'Cancel') : (isAr ? 'إضافة مستخدم جديد' : 'Register New User')}</span>
            </button>
          )}
      ...
      <div className="flex flex-wrap gap-2.5 border-b border-white/5 pb-4 mb-6">
        ...
        >
          ...
          <span>{isAr ? 'عضويات وبصمات الطاقم' : 'Crew Identity & Keys'}</span>
        </button>
        <button
      ...
      {adminSubTab === 'roster' ? (
        ...
        {/* Left Side: Users Roster Selector */}
        <div className="lg:col-span-4 space-y-2">
          <h3 className="text-xs font-semibold text-zinc-400 uppercase tracking-widest mb-3 font-mono text-left">
            {isAr ? 'طاقم العمل المعتمد' : 'Registered Operations Crew'}
          </h3>
          <div className="space-y-2">
            {users.map(u => {
              const isSelected = selectedEditUser?.id === u.id && !isAddingNewUser;
              const uPin = u.pin || fallbackPins[u.id] || '123456';
              const maskedPin = uPin.replace(/./g, '•');
              return (
                <button
                  key={u.id}
                  onClick={() => {
                  ...
                  className={`w-full flex items-center gap-3 p-3 rounded-xl border text-left cursor-pointer transition-all ${
                    isSelected
                      ? 'bg-indigo-500/15 border-indigo-500/40 text-white ring-2 ring-indigo-500/15 shadow-md shadow-indigo-500/5'
                ...
                >
                  ...
                  <div className="truncate flex-1 text-left">
                    <p className={`text-xs font-bold ${isSelected ? 'text-indigo-200' : 'text-white'}`}>{u.name}</p>
                    <span className="text-[9px] text-zinc-500 font-mono tracking-wider block mt-0.5 uppercase">
                      {u.title.split('(')[0]}
                    </span>
                  </div>
                  <div className="shrink-0 flex flex-col items-end gap-1">
                    <span className="text-[8px] px-1.5 py-0.5 rounded bg-emerald-500/10 text-emerald-400 font-bold uppercase tracking-widest font-mono">
                      PIN: {maskedPin}
                    </span>
                  </div>
                </button>
        ...
        <div className="lg:col-span-8">
          {successMsg && (
            <div className="bg-emerald-500/10 border border-emerald-500/30 text-emerald-300 rounded-xl p-3.5 text-xs flex items-center gap-2 mb-4 animate-in slide-in-from-top-1 duration-200">
          ...
          {isAddingNewUser ? (
            /* ADD NEW USER FORM */
            <form onSubmit={handleAddNewUser} className="bg-black/20 border border-white/5 rounded-2xl p-6.5 space-y-4">
              <div className="flex items-center gap-2.5 mb-2.5 text-xs font-bold text-white uppercase text-indigo-300">
                <UserPlus className="h-4 w-4 text-indigo-400 shrink-0" />
                <span>{isAr ? 'إنشاء حساب موظف/فني جديد' : 'Register New Team Member'}</span>
              </div>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {/* Name */}
                <div>
                  <label className="block text-zinc-400 text-xs font-bold mb-1.5 text-left">
                  ...
                  <input
                    ...
                    placeholder={isAr ? 'مثال: أحمد عادل' : 'e.g. Ahmed Adel'}
                    className="w-full bg-[#111116]/80 border border-white/10 rounded-xl px-3.5 py-2.5 text-xs text-white focus:outline-none focus:border-indigo-500/50"
                  />
                </div>
                ...
                <div>
                  <label className="block text-zinc-400 text-xs font-bold mb-1.5 text-left">
                    {isAr ? 'اسم المستخدم (لتسجيل الدخول) *' : 'Username (Credential Switcher) *'}
                  </label>
                  <input
                    type="text"
                    required
                ...
                <div>
                  <label className="block text-zinc-400 text-xs font-bold mb-1.5 text-left">
                    {isAr ? 'كود المرور التوثيقي (PIN) *' : 'Authentication PIN *'}
                  </label>
                  <div className="relative">
                    <input
                      ...
                      placeholder="123456"
                      className="w-full bg-[#111116]/80 border border-white/10 rounded-xl pl-9 pr-10 py-2.5 text-xs text-white font-mono font-bold focus:outline-none focus:border-indigo-500/50"
                    />
                    <KeyRound className="absolute left-3.5 top-3.5 h-4 w-4 text-zinc-500" />
                    <button
                      type="button"
                      onClick={() => setShowNewPin(!showNewPin)}
                      className="absolute right-3.5 top-3.5 text-zinc-500 hover:text-white transition-all cursor-pointer"
                    >
                      {showNewPin ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}
                    </button>
                  </div>
                  <span className="text-[9px] text-zinc-500 block mt-1 font-mono uppercase text-left">
                    {isAr ? 'يجب إدخال 6 أرقام تماثل هوية الدخول' : 'Exactly 6 numeric digits'}
                  </span>
                </div>
                ...
                <div>
                  <label className="block text-zinc-400 text-xs font-bold mb-1.5 text-left">
                    {isAr ? 'الدور الوظيفي وصلاحيات الوصول' : 'System Role & Access Privileges'}
                  </label>
                  <select value={newTitle} onChange={(e) => {
                    const title = e.target.value; setNewTitle(title);
                    setNewRole(title === 'General Manager' ? 'GM' : title.startsWith('Director') ? 'Director' : title.includes('Manager') ? 'Manager' : 'Assistant');
                  }} className="w-full bg-white border border-white/10 rounded-xl px-3.5 py-2.5 text-xs text-slate-900 focus:outline-none focus:border-indigo-500/50">
                    <option value="">{isAr ? 'اختر الموقع الوظيفي الحقيقي' : 'Select actual job position'}</option>
                    <option value="General Manager">{isAr ? 'General Manager — مدير عام' : 'General Manager'}</option>
                    <option value="Director IT">{isAr ? 'Director IT — مدير إدارة تقنية المعلومات' : 'Director IT'}</option>
                    <option value="Director F&B">{isAr ? 'Director F&B — مدير إدارة الأغذية والمشروبات' : 'Director F&B'}</option>
                    <option value="Director Rooms">{isAr ? 'Director Rooms — مدير إدارة الغرف' : 'Director Rooms'}</option>
                    <option value="Director Operations">{isAr ? 'Director Operations — مدير إدارة العمليات' : 'Director Operations'}</option>
                    <option value="IT Manager">{isAr ? 'IT Manager — مدير تقنية المعلومات' : 'IT Manager'}</option>
                    <option value="IT Assistant">{isAr ? 'IT Assistant — مساعد تقنية المعلومات' : 'IT Assistant'}</option>
                  </select>
                </div>
                ...
                <div>
                  ...
                  <select
                    ...
                    className="w-full bg-white border border-white/10 rounded-xl px-3.5 py-2.5 text-xs text-slate-900 focus:outline-none focus:border-indigo-500/50"
                  >
                    <option value="" className="bg-white text-slate-900">{isAr ? 'بدون قسم' : 'No department'}</option>
                    {departments.map(d => (
                ...
                {/* Reports to: keeps the account inside the real hierarchy */}
                <div>
                  <label className="block text-zinc-400 text-xs font-bold mb-1.5 text-left">{isAr ? 'يتبع إلى' : 'Reports To'}</label>
                  <select value={newParentId} onChange={(e) => setNewParentId(e.target.value)} className="w-full bg-white border border-white/10 rounded-xl px-3.5 py-2.5 text-xs text-slate-900">
                    <option value="">{isAr ? 'اختر المسؤول المباشر' : 'Select direct supervisor'}</option>
                ...
                <div>
                  <label className="block text-zinc-400 text-xs font-bold mb-1.5 text-left">
                    {isAr ? 'البريد الإلكتروني' : 'Professional Email'}
                  </label>
                  <input
                    type="email"
                    value={newEmail}
                    onChange={(e) => setNewEmail(e.target.value)}
                    placeholder="name@longbeach-resort.com"
                    className="w-full bg-[#111116]/80 border border-white/10 rounded-xl px-3.5 py-2.5 text-xs text-white focus:outline-none focus:border-indigo-500/50"
                  />
                </div>
                ...
                {/* Position is selected above from the real organization structure */}
                <div className="md:col-span-2">
                  <p className="text-[10px] text-zinc-500">{isAr ? 'الموقع الوظيفي المحدد من القائمة هو الذي يحدد الصلاحيات.' : 'The selected job position determines the account permissions.'}</p>
                </div>

                {/* Skills */}
                <div className="md:col-span-2">
                  <label className="block text-zinc-400 text-xs font-bold mb-1.5 text-left">
                    {isAr ? 'المهارات الفنية الأساسية (مفصولة بفاصلة)' : 'Technical Core Skills (comma-separated list)'}
                  </label>
                  <input
                    type="text"
                    value={newSkillsStr}
                    onChange={(e) => setNewSkillsStr(e.target.value)}
                    placeholder="e.g. VLAN, Cisco CallManager, Virtualization"
                    className="w-full bg-[#111116]/80 border border-white/10 rounded-xl px-3.5 py-2.5 text-xs text-white focus:outline-none focus:border-indigo-500/50"
                  />
                </div>
              </div>
              ...
              <div className="space-y-3">
                <label className="block text-zinc-400 text-xs font-bold text-left">
                  {isAr ? 'الصورة التعبيرية للملف' : 'Identity avatar'}
                </label>
                
                <div className="flex flex-col sm:flex-row items-center gap-4 bg-[#111116]/40 border border-white/5 p-4 rounded-xl">
                  <div className="relative h-14 w-14 rounded-full overflow-hidden border border-indigo-500/50 shrink-0">
                    <img src={newAvatar} alt="New Preview" className="h-full w-full object-cover" referrerPolicy="no-referrer" />
                  </div>
                  <div className="flex-1 w-full text-left">
                    <span className="block text-[11px] text-zinc-400 mb-1.5 font-semibold">
                      {isAr ? 'اختر صورة من جهازك الشخصي:' : 'Select image from your personal device:'}
                    </span>
                    <input
                      type="file"
                    ...
                    <label
                      ...
                      className="inline-flex items-center gap-2 px-4 py-2 border border-dashed border-zinc-700 hover:border-indigo-500/50 hover:bg-indigo-500/10 rounded-xl cursor-pointer text-xs text-zinc-300 hover:text-white transition-all font-semibold"
                    >
                      <Upload className="h-4 w-4 shrink-0 text-indigo-400" />
                      <span>{isAr ? 'اختر من الملفات...' : 'Browse files...'}</span>
                    </label>
                  </div>
                </div>

                <div className="text-zinc-500 text-[10px] uppercase font-mono tracking-wider text-left">
                  {isAr ? 'أو اختر من الرموز الجاهزة:' : 'Or choose from presets:'}
                </div>

                <div className="grid grid-cols-4 sm:grid-cols-8 gap-2 bg-[#111116]/80 border border-white/10 rounded-xl p-3">
                  {PRESET_AVATARS.map((preset, idx) => {
                    const isSelectedPreset = newAvatar === preset;
                    return (
                      <button
                        key={idx}
                        type="button"
                        onClick={() => setNewAvatar(preset)}
                        className={`relative rounded-lg overflow-hidden h-10 w-full border-2 transition-all cursor-pointer ${
                          isSelectedPreset ? 'border-indigo-500 scale-105 shadow-md shadow-indigo-500/10' : 'border-transparent opacity-60 hover:opacity-100'
                        }`}
                      >
              ...
              <div className="pt-3 border-t border-white/5 flex items-center justify-end gap-3">
                <button
                  ...
                  className="bg-zinc-800 hover:bg-zinc-700 text-zinc-300 font-semibold text-xs px-5 py-2.5 rounded-xl cursor-pointer transition-all"
                >
                  {isAr ? 'إلغاء الأمر' : 'Cancel'}
                </button>
                <button
                  type="submit"
                  className="bg-emerald-600 hover:bg-emerald-500 text-white font-semibold text-xs px-5 py-2.5 rounded-xl cursor-pointer shadow-md transition-all shrink-0 flex items-center gap-1.5"
                >
                  <UserPlus className="h-3.5 w-3.5 shrink-0" />
                  <span>{isAr ? 'إنشاء وتوثيق الحساب الجديد' : 'Register New User'}</span>
                </button>
              </div>
            </form>
          ) : selectedEditUser ? (
            /* EDIT EXISTING USER FORM */
            <form onSubmit={handleSave} className="bg-black/20 border border-white/5 rounded-2xl p-6.5 space-y-4">
              <div className="flex items-center justify-between gap-4 mb-2.5 text-xs font-bold text-white uppercase text-indigo-300">
                <div className="flex items-center gap-2.5">
                  <UserSquare2 className="h-4 w-4 text-indigo-400" />
                  <span>{isAr ? `تحديث الملف السري: ${selectedEditUser.name}` : `Modify Identity & Token: ${selectedEditUser.name}`}</span>
                </div>

                {selectedEditUser.id !== currentUser.id && (
                  <div className="relative">
                    {deleteConfirmId === selectedEditUser.id ? (
                      <div className="flex items-center gap-2 animate-in fade-in duration-200">
                        <span className="text-[10px] text-rose-400 capitalize font-mono shrink-0">
                          {isAr ? 'تأكيد الحذف نهائياً؟' : 'Are you sure?'}
                        </span>
                        <button
                          type="button"
                          onClick={() => handleDeleteUser(selectedEditUser.id)}
                          className="bg-rose-600 hover:bg-rose-500 text-white font-bold text-[10px] px-2.5 py-1 rounded-lg shrink-0 transition-all cursor-pointer"
                        >
                        ...
                        <button
                          ...
                          className="bg-zinc-700 hover:bg-zinc-600 text-zinc-300 font-bold text-[10px] px-2 py-1 rounded-lg shrink-0 transition-all cursor-pointer"
                        >
                          {isAr ? 'تراجع' : 'No'}
                        </button>
                      </div>
                      ...
                      <button
                        type="button"
                        onClick={() => setDeleteConfirmId(selectedEditUser.id)}
                        className="bg-rose-500/10 hover:bg-rose-500/20 text-rose-400 hover:text-rose-300 border border-rose-500/25 px-2.5 py-1 rounded-lg text-[10px] font-bold uppercase tracking-wider flex items-center gap-1 transition-all cursor-pointer"
                        title={isAr ? 'مسح هذا العضو بالكامل' : 'Delete Member Registry'}
                      >
                        <Trash2 className="h-3 w-3" />
                        <span>{isAr ? 'حذف الحساب' : 'Delete User'}</span>
                      </button>
                    )}
                  </div>
                )}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-zinc-400 text-xs font-bold mb-1.5 text-left">
                    {isAr ? 'الاسم بالكامل' : 'Full Professional Name'}
                  </label>
                  <input
                    type="text"
                    value={name}
                    onChange={(e) => setName(e.target.value)}
                    placeholder={selectedEditUser.name}
                    className="w-full bg-[#111116]/80 border border-white/10 rounded-xl px-3.5 py-2.5 text-xs text-white focus:outline-none focus:border-indigo-500/50"
                  />
                </div>

                {/* PIN Code Security Passphrase */}
                <div>
                  <label className="block text-zinc-400 text-xs font-bold mb-1.5 text-left">
                    {isAr ? 'كود المرور التوثيقي (PIN)' : 'Security Authentication PIN'}
                  </label>
                  <div className="relative">
                    <input
                    ...
                    <button
                      ...
                      className="absolute right-3.5 top-3 text-zinc-500 hover:text-white transition-all cursor-pointer"
                    >
                      {showPin ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}
                    </button>
                  </div>
                  <span className="text-[9px] text-zinc-500 block mt-1 font-mono uppercase text-left">
                    {isAr ? 'يجب إدخال 6 أرقام كحد أقصى' : 'Input range restricted to digits [0-9] (length: 6)'}
                  </span>
                </div>
                ...
                <div>
                  ...
                  <select
                    value={selectedEditUser.departmentId || ''}
                    onChange={(e) => handleAssignUserDepartment(selectedEditUser.id, e.target.value)}
                    className="w-full bg-white border border-white/10 rounded-xl px-3.5 py-2.5 text-xs text-slate-900 focus:outline-none focus:border-indigo-500/50"
                  >
                    <option value="" className="bg-white text-slate-900">{isAr ? 'بدون قسم' : 'No department'}</option>
                    {departments.map(d => (
              ...
              {/* Avatar Image Selection Preset & Field */}
              <div className="space-y-3">
                <label className="block text-zinc-400 text-xs font-bold text-left">
                  {isAr ? 'الصورة الشخصية (تكامل الأفتار)' : 'Staff Representation avatar'}
                </label>
                ...
                <div className="flex flex-col sm:flex-row items-center gap-4 bg-[#111116]/40 border border-white/5 p-4 rounded-xl">
                  {avatar ? (
                    <div className="relative h-14 w-14 rounded-full overflow-hidden border border-indigo-500/50 shrink-0">
                      <img src={avatar} alt="Preview" className="h-full w-full object-cover" referrerPolicy="no-referrer" />
                    </div>
                  ) : (
                    <div className="h-14 w-14 rounded-full bg-zinc-800 border border-white/10 flex items-center justify-center text-zinc-500 text-xs shrink-0 font-bold">
                      {isAr ? 'بدون' : 'None'}
                    </div>
                  )}
                  <div className="flex-1 w-full text-left">
                    ...
                    <input
                      type="file"
                      id="avatar-device-upload"
                      accept="image/*"
                      onChange={(e) => handleFileChange(e, 'edit')}
                      className="hidden"
                    />
                    <label
                      htmlFor="avatar-device-upload"
                      className="inline-flex items-center gap-2 px-4 py-2 border border-dashed border-zinc-700 hover:border-indigo-500/50 hover:bg-indigo-500/10 rounded-xl cursor-pointer text-xs text-zinc-300 hover:text-white transition-all font-semibold"
                    >
                      <Upload className="h-4 w-4 shrink-0 text-indigo-400" />
                      <span>{isAr ? 'اختر من الملفات...' : 'Browse files...'}</span>
                ...
                <div className="grid grid-cols-4 sm:grid-cols-8 gap-2 bg-[#111116]/80 border border-white/10 rounded-xl p-3">
                  {PRESET_AVATARS.map((preset, idx) => {
                    const isSelectedPreset = avatar === preset;
                    return (
                      <button
                        key={idx}
                        type="button"
                ...
                <div className="relative">
                  <input
                    ...
                    placeholder={isAr ? 'رابط الصورة (URL أو ترميز Base64 التلقائي)...' : 'Avatar URL / base64 code...'}
                    className="w-full bg-[#111116]/80 border border-white/10 rounded-xl px-3.5 py-2.5 text-xs text-white placeholder-zinc-600 focus:outline-none focus:border-indigo-500/50"
                  />
                </div>
              </div>

              <div className="pt-3 border-t border-white/5 flex items-center justify-between gap-4">
                <div className="text-[10px] text-zinc-500 italic max-w-sm text-left">
                  {isAr
                    ? 'بعد تفعيل التحديث، سينطبق الرمز المروري الجديد فوراً لتسجيل الدخول بأمان.'
                    : 'Modifying credential matrices triggers real-time cloud synchronisation instantly.'}
                </div>
                <button
                  type="submit"
                  className="bg-indigo-600 hover:bg-indigo-500 text-white font-semibold text-xs px-5 py-2.5 rounded-xl cursor-pointer shadow-md transition-all shrink-0 flex items-center gap-1.5"
                >
                  <RefreshCw className="h-3.5 w-3.5 shrink-0" />
                  <span>{isAr ? 'تأكيد وحفظ التحديث' : 'Commit Configuration'}</span>
                </button>
              </div>
            </form>
          ) : (
            <div className="h-full min-h-[220px] bg-black/20 border border-white/5 rounded-2xl flex flex-col items-center justify-center p-6 text-zinc-500">
              <Shield className="h-8 w-8 text-zinc-600 mb-2 animate-pulse" />
              <p className="text-xs text-center font-mono">
                {isAr ? 'الرجاء اختيار فني صيانة من الخيارات الجانبية لتنشيط اللوحة.' : 'Select a crew operator profile to activate secure configuration controls.'}
              </p>
            </div>
          )}
        </div>
